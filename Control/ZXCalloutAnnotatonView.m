...
@interface ZXCalloutAnnotatonView(){
...
@implementation ZXCalloutAnnotatonView
...
+(instancetype)calloutViewWithMapView:(MKMapView *)mapView{
    ...
    ZXCalloutAnnotatonView *calloutView=(ZXCalloutAnnotatonView *)[mapView dequeueReusableAnnotationViewWithIdentifier:calloutKey];
    if (!calloutView) {
        calloutView=[[ZXCalloutAnnotatonView alloc]init];
    }
    return calloutView;
}

#pragma mark 当给大头针视图设置大头针模型时可以在此处根据模型设置视图内容
-(void)setAnnotation:(ZXAnnotation *)annotation{
